...
def _resolve_literal(
    ...
    """
    return values + [StrProxy(command_config, meta=statements_meta)]
...
def _resolve_entity(
    ...
    return values + [StrProxy(generate_pseudo_id(command_config), meta=statements_meta)]
...
def _resolve_column(
    ...
    return values + [StrProxy(val, meta=statements_meta) for val in jmespath_results_as_array(command_config, record)]
...
def _resolve_template(
    ...
    return values + [StrProxy(template.render(entity=entity.properties, record=record), meta=statements_meta)]

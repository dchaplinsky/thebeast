...
class ProxyTestCase(TestCase):
    ...
    def test_adding_meta(self):
        ...
        self.assertEqual(len(pers.get("name")), 2)

        for val in pers.get("name"):
            self.assertTrue(isinstance(val, StrProxy))
            if val == "foobar":
                self.assertEqual(val._meta.locale, "uk")
                self.assertEqual(val._meta.date, None)

        parent_pers = EntityProxy(model, {"schema": "Person"}, key_prefix="key_prefix")
        parent_pers.make_id("foo", "bar")

        pers.add("parent", parent_pers)
        self.assertEqual(pers.get("parent"), [parent_pers.id])
